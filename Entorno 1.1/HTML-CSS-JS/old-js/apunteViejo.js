Array.from(variable) //Convierte en array un elemento iterable (muestra cada caracter)

    variable.sort([callback]) //Ordena los elementos de un array alfabeticamente (valor Unicode). Un callback es una funcion que la ejecuta otra funcnion.

        variable.sort((a,b)=>a-b) //Ordena de menor a mayor numeros, "b-a" ordena de mayor a menor

    unArray.forEach((referencia)=> console.log(referencia)); //Ejecuta la funcion para cada elemento del array, no es obligatorio el indice

    unArray.forEach((element,index) =>
        console.log(`${element} está en la posicion ${index}`));

    unArray.maps(callback) //Transforma todos los elementos de un array y devuelve un nuevo array

        const number = [1,2,3]
        number.maps(referencia => console.log(referencia*2))

    unArray.filter() //genera un filtro en base a una condicion

        numeros.filter(number => number > 40)

    unArray.reduce() //Trabaja por pares de elementos, devuelve un unico valor

        console.log(numeros.reduce((a,b) => a+b));// suma todos los numeros y devuelve un numero
...
//Otro ejemplo
        ...
        const userOnline = usuarios.reduce((contador,user) =>{
            if (user.online) contador++
            return contador
        },0) // ,nuestro contador inicia en 0
...
// Parámetros Rest
            ...
            const calc = (a,...x) => {
                let numberA = a;
                x.forEach(x => {         //Cada parametro se suma al anterior
                    numberA += x;
                });
                return numberA
            }
            console.log(calc(5,5,1))
